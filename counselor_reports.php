...
<div class="modal fade" id="progressChartModal" tabindex="-1" aria-labelledby="progressChartLabel" aria-hidden="true">
    <div class="modal-dialog modal-fullscreen modal-dialog-centered"> <!-- Use modal-fullscreen -->
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="progressChartLabel">📊 Progress Chart</h5>
            ...
            <div class="modal-body">
                ...
                <div class="card shadow mb-4">
                  <div class="card-body">
                    <canvas id="resultsChart" height="100"></canvas> <!-- Adjust canvas height if needed -->
                  </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function showProgressChart(results) {
        ...
        results.forEach(entry => {
            labels.push(entry.date_taken);
            depressionScores.push(entry.depression_score);
            anxietyScores.push(entry.anxiety_score);
            stressScores.push(entry.stress_score);
        });

        const ctx = document.getElementById('resultsChart').getContext('2d');

        // Destroy existing chart to avoid overlapping
        if (resultsChart) {
        ...
        resultsChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Mental Health Test Progress'
                    }
                },
                scales: {
                    y: {
                        beginAtZero: true,
                        max: 42 // Assuming max DASS score, adjust if needed
                    }
                }
            }
        });
